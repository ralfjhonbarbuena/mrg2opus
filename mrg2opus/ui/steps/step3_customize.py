...
def render(state: WizardState) -> None:
    ...
    include_vertical_rates = st.checkbox(
        ...
        help=(
            "Adds a VERTICAL RATES sheet: the same rates as OPUS RATES, reshaped one row per container size "
            "instead of 4 rate columns per row. A general OPUS upload option (not lane-specific) - faster to "
            "upload but capped at 10,000 rows per file, so it isn't generated by default."
        ),
    )

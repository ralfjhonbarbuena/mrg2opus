...
class TADWmwWewParser(BaseMRGParser):
    ...
    def parse_raw(self, wb: Workbook) -> RawExtraction:
        ...
        if SHEET_WEW in wb.sheetnames:
            scopes[SHEET_WEW] = _read_sheet(wb[SHEET_WEW], SHEET_WEW)
        if SHEET_WMW in wb.sheetnames:
            scopes[SHEET_WMW] = _read_sheet(wb[SHEET_WMW], SHEET_WMW)
